...
namespace TimeEffort.DAL
{
    public class AllDbServices
    {
       private static ErpSystemContext db;
       ...
       private void ContextSet()
       {
        ...
        {
            using (ErpSystemContext db = new ErpSystemContext())
            {
        ...
        public Position GetUserRoleByUser(string username)
        {
            using(ErpSystemContext db = new ErpSystemContext())
            {
                return db.UserInfo.FirstOrDefault(u => u.Username == username).Position;
            }
        }
